...
var NTLSPro = {
  ...
  websocket_init: function () {
    ...
    if(!NTLSPro.websocket_url){
    	NTLSPro.websocket_url='wss://127.0.0.1:10022'; //默认值
	}
    ...
    wss_obj.onopen = function (event) {
    	...
    	var len = $('#websocket_connect').length;
    	if (len>0){
    		$('#websocket_connect').attr('disabled',true);
    		$('#websocket_connect').addClass('websocket_connect_disabled');
    	}
    };
    ...
    wss_obj.onclose = function (event) {
    	...
    	var len = $('#websocket_connect').length;
    	if (len>0){
    		$('#websocket_connect').attr('disabled',false);
    		$('#websocket_connect').removeClass('websocket_connect_disabled');
    	}
    };
    ...
    function websocket_connect_error() {
        ...
        //check_ntls_timer = setTimeout('NTLSPro.websocket_init()', 3000);
    }
  }
...
//写日志
function write_log(log,status){
	var len = $('#ntls_server_check_result').length;
	if (len>0){
		//$('#ntls_server_check_result').append($.trim(log)+"\n");
		if(arguments[1]===true){
			$('#ntls_server_check_result').append("<font color='green'>"+$.trim(log)+"</font></br></br>");
		}else if(arguments[1]===false){
			$('#ntls_server_check_result').append("<font color='red'>"+$.trim(log)+"</font></br></br>");
		}else{
